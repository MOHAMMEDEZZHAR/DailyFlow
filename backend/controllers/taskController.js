...
export const addTask = async (req, res, next) => {
  try {
    const { name, duration, priority, description, categorie } = req.body;
    if (!name || !duration) {
      return res.status(400).json({ message: 'Nom et durée requis' });
    }
    const taskId = await createTask({ name, duration, priority, user_id: req.user.id, description, categorie });
...
export const updateTaskCtrl = async (req, res, next) => {
  try {
    ...
    await updateTask(taskId, { name, duration, priority, description, categorie });
